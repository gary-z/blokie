"use strict";

// The assist's move planning. The solver underneath only ever plans moves that
// place every piece it is given, so what is checked here is what happens when
// no such move exists: the assist has to keep playing what does fit, right up
// to the point the game is genuinely over.

import { blokie, bits, init } from '../../engine/js/blokie.js';

/** @typedef {import('../../engine/js/blokie.js').Hand} Hand */
/** @typedef {import('../../engine/js/blokie.js').Game} Game */
/** @typedef {import('../../engine/js/blokie.js').BitBoard} BitBoard */

await init();

let failures = 0;
/** @type {(condition: boolean, description: string) => void} */
function check(condition, description) {
    if (!condition) {
        failures++;
        console.error("FAIL: %s", description);
        return;
    }
    console.log("ok - %s", description);
}

/** @type {(board: BitBoard, score?: number) => Game} */
function gameWithBoard(board, score = 0) {
    return {
        board: board,
        previous_move_was_clear: false,
        score: score,
    };
}

// A real position, reached by playing pieces at random until the solver gave
// up. Two of the three slots hold something, the first of them fits nowhere,
// and the last one fits in exactly one place.
//
// .........  ..#######  ###.#####  .######.#  ..#.##...
// ###.#.#..  .##.#####  ....#.###  ....#....
const STUCK_BOARD = { a: 132118528, b: 22833534, c: 4432374 };
/** @type {Hand} */
const STUCK_HAND = [
    { a: 1054720, b: 12, c: 0 },
    { a: 0, b: 0, c: 0 },
    { a: 3670016, b: 4, c: 0 },
];

check(blokie.hasValidMove(STUCK_BOARD, STUCK_HAND),
    "the fixture is a position the game is not over in");
check(!blokie.canPlace(STUCK_BOARD, STUCK_HAND[0])
    && blokie.canPlace(STUCK_BOARD, STUCK_HAND[2]),
    "the fixture holds one piece that fits and one that does not");

const stuck_game = gameWithBoard(STUCK_BOARD);

// What the bug looked like: asked for the whole hand, the solver can place
// nothing at all, and says so.
const whole_hand = blokie.makeMove(stuck_game, STUCK_HAND);
check(!whole_hand.found && whole_hand.moves.length === 0,
    "the solver places nothing when it cannot place every piece");
check(whole_hand.game === stuck_game,
    "and hands back the game it was given, rather than a board it made up");

const plan = blokie.plan(stuck_game, STUCK_HAND);
check(plan.length === 1, "the plan falls back to the one piece that does fit");
check(plan.length === 1 && plan[0].piece_index === 2,
    "the plan names the slot the piece came out of");
check(plan.length === 1
    && blokie.place(stuck_game, plan[0].placement) !== null,
    "the planned placement is legal");

// A plan for a board with room on it still places all three, in three slots.
const open_hand = blokie.deal();
const open_plan = blokie.plan(blokie.newGame(), open_hand);
check(open_plan.length === 3, "an open board is still planned three pieces at a time");
check(new Set(open_plan.map(m => m.piece_index)).size === open_plan.length,
    "a plan uses each hand slot at most once");

// Playing the fixture out: the assist has to reach a position where nothing on
// hand fits, rather than stalling on the first hand it cannot fully place. The
// hand is never refilled here, so this is a handful of moves either way.
let game = stuck_game;
// Copied rather than shared, since the loop below plays slots out of it. `map`
// gives an array back and a hand is three slots exactly, so it is spelled here.
let hand = /** @type {Hand} */ (STUCK_HAND.map(p => ({ ...p })));
let moves = 0;
let every_placement_legal = true;
for (;;) {
    const next = blokie.plan(game, hand);
    if (next.length === 0) {
        break;
    }
    for (const move of next) {
        const result = blokie.place(game, move.placement);
        if (result === null) {
            every_placement_legal = false;
            break;
        }
        hand[move.piece_index] = bits.empty();
        game = result.new_game;
        moves++;
    }
    if (!every_placement_legal || moves > 10) {
        break;
    }
}
check(moves > 0, "the assist plays on from a position it used to give up in");
check(every_placement_legal, "every placement it played was legal");
check(!blokie.hasValidMove(game.board, hand),
    "it plays until nothing in hand fits, which is the game being over");

console.log(failures === 0 ? "\nall checks passed" : `\n${failures} check(s) failed`);
process.exit(failures === 0 ? 0 : 1);
